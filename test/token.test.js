  ...
  describe('once minted', () => {
    ...
    describe('can record dividends', () => {
      ...
      it('and keeps track of holders when minting and burning', async () => {
        await await token.getNumTokenHolders().should.eventually.eq(2)  // be my

//      await assertHolderList(token, accounts[0], accounts[1])

        await token.mint({ value: 100, from: accounts[2] })
        await await token.getNumTokenHolders().should.eventually.eq(3)  // be my
        await token.burn(accounts[9])
        await await token.getNumTokenHolders().should.eventually.eq(4)  // be my
